...
import CV from "./CADViewerCanvas_no_npm.vue";
...
function cadviewerCanvasMethod01(){
  ...
  cvjs_executeCustomCanvasMethod_drag(generic_start_method_01, generic_stop_method_01, generic_move_method_01,'')

}

function cadviewerCanvasMethod02(){

  cvjs_executeCustomCanvasMethod_click(generic_mousemove_method_01, generic_mousedown_method_01, generic_mouseup_method_01, generic_dblclick_method_01,'')
...
var generic_make_rect_arrow_stop_method1 = function() {
  ...
  // create a group with space as main object
  cvjs_createSpaceObjectGroup( currentNode_underbar, cvjs_RubberBand, "API"+randomNr, "API"+randomNr, "Generic", "myLayer", "myGroup");   
...
//////////  DECORATE TEXT ON SPACES
		...
		customAddTextToSpaces(){
			...
			for (var spc in spaceObjectIds)
			{
				...
				if ((i % 3) ==0){
					...
					//cvjs_AddTextOnSpaceObject(CV.textLayer1, spaceObjectIds[spc],  0.05, textString, textStyles, scaleText, hexColorText, true, false);
          ...
          // SINCE WE ARE USING THE textLayer1 on CADViewerCanvas, we use eventBus to pass the method over
						// here we clip the roomlables so they are inside the room polygon            
						//cvjs_AddTextOnSpaceObject(myGlobalTextLayer /*CV.textLayer1*/, spaceObjectIds[spc],  0.1, textString, textStyles, scaleText, hexColorText, true, false);
          ...
          // SINCE WE ARE USING THE textLayer1 on CADViewerCanvas, we use eventBus to pass the method over
						...
						//cvjs_AddTextOnSpaceObject(myGlobalTextLayer /*CV.textLayer1*/, spaceObjectIds[spc],  0, textString, textStyles, scaleText, hexColorText, true, true);
		...
		hide_object_in_group(){
			...
			var node = cvjs_getSpaceObjectNodefromId(id);
			var objectTag = jQuery('#group_2_subid').val();    
			cvjs_hideObjectInSpaceObjectGroup(node, objectTag);
		...
		show_object_in_group(){
			...
			var node = cvjs_getSpaceObjectNodefromId(id);
			var objectTag = jQuery('#group_2_subid').val();    
			cvjs_showObjectInSpaceObjectGroup(node, objectTag);
